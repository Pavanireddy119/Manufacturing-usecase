"""
Train YOLOv8s on the fixed dataset (dataset_yolo_fixed/).
...
def main():
    ...
    os.makedirs('output/models_fixed', exist_ok=True)
    os.makedirs('output/reports_fixed', exist_ok=True)
    os.makedirs('output/predictions_fixed', exist_ok=True)
    
    # Copy weights
    for w in ['best.pt', 'last.pt']:
        src = train_dir / 'weights' / w
        if src.exists():
            shutil.copy(src, f'output/models_fixed/{w}')
            print(f"  Copied {w}")
    ...
    for fname in ['confusion_matrix.png', 'PR_curve.png', 'P_curve.png', 'R_curve.png', 
        ...
        src = train_dir / fname
        if src.exists():
            shutil.copy(src, f'output/reports_fixed/{fname}')
            print(f"  Copied {fname}")
    
    print(f"\nOutput files:")
    print(f"  - Models: output/models_fixed/")
    print(f"  - Reports: output/reports_fixed/")
    print(f"  - Predictions: output/predictions_fixed/")
